...
export default async function handler(req, res) {
  ...
  try {
    ...
    const mr = await fetch(SUPABASE_URL + '/rest/v1/rpc/ged_match', {
      ...
      body: JSON.stringify({ query_embedding: '[' + emb.join(',') + ']', match_count: Math.min(match_count || 8, 20) })
    });
    ...
    const userMsg = "EXTRAITS DE DOCUMENTS (les plus pertinents trouvés dans la base) :\n\n" + context + "\n\nQUESTION : " + question;
