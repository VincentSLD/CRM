...
- GPH (codes GPH64 et GPH85) — Bureau d'études GÉOTECHNIQUE (études de sol) et STRUCTURE dédié à la MAISON INDIVIDUELLE (depuis 2011). ~70 collaborateurs, agences Challans, Nantes, Niort, Pau ; spécialiste PARASISMIQUE. Double compétence sol + structure (cohérence terrain/fondations). Missions par étape : (1) À la parcelle — étude de sol G1 (via REGAR, filiale dédiée au G1 préalable, obligatoire loi ELAN en zone argileuse), G1 PGC lotissement, étude d'assainissement (avec OCE), bornage ; (2) Projet neuf — étude de sol G2 AVP et G2 PRO (obligatoire CCMI), étude structure gros œuvre (fondations, dallage, béton armé, Eurocodes/EC8), charpente bois (avec ECTS), attestations sismique & retrait-gonflement des argiles (obligatoires depuis 2024) ; (3) Diagnostic/rénovation — diagnostic géotechnique G5 (fissures, tassements), diagnostic structure (capacité portante avant surélévation/extension/PV), études de rénovation (murs porteurs, trémies, renforcement planchers). Études de STRUCTURE : France entière + DOM-TOM ; études de SOL (terrain) : agences Grand Ouest et Sud-Ouest. Cibles : particuliers, constructeurs de maisons individuelles (CMI), lotisseurs, architectes. Voir aussi REGAR (entité dédiée au G1).
- REGAR (code GPH-R, site gph-regar.fr) — Bureau d'études GÉOTECHNIQUE, société PRODUCTRICE du groupe (entité à part entière, NE PAS la considérer comme une holding ni comme non productive). REGAR fait UNIQUEMENT des études de sol G1 loi ELAN À LA PARCELLE, pour la VENTE d'une parcelle constructible (typiquement un lot de lotissement), et UNIQUEMENT en zone d'aléa argile MOYEN ou FORT (retrait-gonflement des argiles / RGA). (Pas de G1 PGC global, pas de G2/G5 ni de structure : ces missions relèvent de GPH ou GRAVITY — voir la répartition ci-dessous.) Modèle commercial : REGAR travaille via des PRESCRIPTEURS — agences immobilières, notaires, géomètres, mairies — qui orientent les vendeurs de terrains vers l'étude G1. Intervention autonome sur site (prélèvements/analyses), rapport transmis par email. Zone d'intervention : Ouest et Sud-Ouest (44, 49, 85, 79, 86, 17, 16, 40, 64, 65, 32, 33, 24, 47). Pour être force de proposition sur REGAR : cibler/animer ces prescripteurs sur les communes en aléa argile moyen/fort.
- RÉPARTITION DES MISSIONS GÉOTECHNIQUES SUR UN LOTISSEMENT (important, ne pas confondre les entités) :
  • REGAR = G1 loi ELAN À LA PARCELLE (vente d'un lot), SEULEMENT en zone argile MOYEN ou FORT.
  • GPH85 et GPH64 = G1 PGC du lotissement dans sa GLOBALITÉ (toutes zones d'argile, y compris faible) ; et À LA PARCELLE, GPH réalise les études G2.
  • GRAVITY = étude de VOIRIE du lotissement (toutes zones d'argile), en plus de son offre géotechnique complète G1-G5 tous ouvrages/infrastructures.
